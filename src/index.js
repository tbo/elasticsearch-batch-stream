...
function bulkWriteStream (options = {}) {
  ...
  writeStream._write = function (chunk, encoding, callback) {
    const body = chunk.reduce((prev, curr) => {
      if (['index', 'update', 'upsert', 'delete'].indexOf(curr.action) < 0) { return prev }
      ...
      if (curr.action === 'upsert') { prev.push({ update: { _index: curr.index, _type: curr.type, _id: curr.id } }, { doc: curr.doc, doc_as_upsert: true }) }
